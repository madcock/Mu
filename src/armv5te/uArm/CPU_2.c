			...
			case 9:		//load/store multiple
				...
				for(vc8 = 0; vc8 < 16; vc8++){
					...
					if(v16 & (1UL << vb8)){
						...
						UInt32 tmpSpace, *reg = cpu->regs + vb8;
						...
						else if (!L && vb8 == 15){		//handle STM(1)'s use of PC
								
							tmpSpace = cpuPrvGetReg(cpu, 15, wasT, specialPC);
							reg  = &tmpSpace;
...
static Err cpuPrvCycleThumb(ArmCpu* cpu){
	...
	UInt32 t, instr = 0xE0000000UL /*most likely thing*/, pc;
	UInt16 instrT, v16;
	UInt8 v8, fsr;
